...
class Polynomials(SlideBase):
    ...
    def construct(self):
        ...
        self.subtract_z_roots = MathTex("R(x)", "=", "3", "(x-z)", "(x-z_1)", "(x-z_2)", color = PRIMARY_COLOR).next_to(self.line_subtract, DOWN, aligned_edge=LEFT)
    ...
    def animate_in(self, scene):
        ...
        scene.play(Create(self.chart1), TransformMatchingShapes(self.chart.graph3, self.chart.graph), Unwrite(self.label_poly_r), FadeOut(self.intersect1, self.intersect2, self.intersect_sub, self.subtract_z_roots), run_time=2)
        scene.wait(4)
        ...
        scene.play(TransformMatchingShapes(self.polynomial0_modulo5, self.polynomial1_modulo5))
