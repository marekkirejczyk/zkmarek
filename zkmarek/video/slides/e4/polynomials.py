...
class Polynomials(SlideBase):
    ...
    def construct(self):
        ...
        self.chart1.to_edge(RIGHT).scale(0.8).shift(UP*0.5)
        self.chart2 = DiscreetePolynomialChart(23, poly)
        self.chart2.to_edge(RIGHT).scale(0.8).shift(UP*0.5)
        self.chart3 = DiscreetePolynomialChart(41, poly)
        self.chart3.to_edge(RIGHT).scale(0.8).shift(UP*0.5)
        ...
        self.polynomial_z = MathTex(r"P({{z}}) = 4 {{z^3}} - 8{{z^2}} - 17 {{z}} + 30 {{=y}}", color = PRIMARY_COLOR).to_edge(RIGHT).shift(LEFT*0.8)
        self.polynomial_brakdown = MathTex("P({{x}})", "=", "4 {{x^3}}", "-", "8{{x^2}}", "-", "17 {{x}}", "+", "30", color = PRIMARY_COLOR).to_edge(RIGHT).shift(LEFT*0.8)
        ...
        self.chart = Chart(include_details=True).scale(0.8).next_to(self.title_label, DOWN).shift(DOWN)
        ...
        self.polynomial_modulo = MathTex(r"P({{x}}) = 4 {{x^3}} - 8{{x^2}} - 17 {{x}} + 30 {{\mod \ p}}")
    def animate_in(self, scene):
        ...
        self.root1.label.shift(UP*0.2+LEFT)
        self.root2.label.shift(DOWN*0.2+LEFT)
        ...
        self.chart_group.target.shift(LEFT*4)
        ...
        self.minus = MathTex("-", color = PRIMARY_COLOR).next_to(self.polynomial_brakdown, LEFT, buff = 0.3).shift(UP*0.3)
        ...
        self.intersect_sub.set_color(PRIMARY_COLOR).shift(DOWN*0.4)
        self.intersect_sub.label.shift(LEFT*0.2)
