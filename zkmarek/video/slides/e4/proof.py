...
class Proof1(SlideBase):
    ...
    def construct(self):
        ...
        self.equality_quotient = MathTex(r"{{q(\tau)}} = {{p(\tau)}}", color = SECONDARY_COLOR).to_edge(RIGHT).shift(LEFT)
        ...
        self.opening3 = MathTex(r"{{p(x)}} {{- y}} = {{r(x)}} {{}}", color = SECONDARY_COLOR, font_size=40)
        self.opening4 = MathTex(r"{{p(z)}} {{- y}} = {{r(z)}} {{=0}}", color = SECONDARY_COLOR, font_size=40)
        self.opening5 = MathTex(r"{{p(x)}} {{- y}} = {{r(x)}} {{(x-z)}} {{(...)}}", color = SECONDARY_COLOR, font_size=40)
        self.opening6 = MathTex(r"{{p(x)}} {{- y}} = {{r(x)}} {{(x-z)}} {{q(x)}}", color = SECONDARY_COLOR, font_size=40)
        ...
        self.opening.to_edge(RIGHT).shift(LEFT*2)
    ...
    def animate_in(self, scene):
        ...
        self.equality_quotient.target.to_edge(DOWN+RIGHT)
        scene.play(MoveToTarget(self.equality_quotient))
        ...
        scene.wait(2)
        scene.play(Indicate(self.equality_quotient[1], color = HIGHLIGHT_COLOR))
        scene.wait(2)
