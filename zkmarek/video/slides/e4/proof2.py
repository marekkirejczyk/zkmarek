...
class Proof2(SlideBase):
    ...
    def animate_in(self, scene):
        ...
        bubble_verifier = RoundedRectangle(corner_radius=0.5, width=speech_text_verifier.width + 1, height=speech_text_verifier.height + 1.2, color = PRIMARY_COLOR).next_to(self.verifier, DOWN+LEFT, buff = -1).shift(0.2*LEFT)
        ...
        bubble_opening = RoundedRectangle(corner_radius=0.5, width=self.opening.width + 0.7, height=self.opening.height + 1.5, color = PRIMARY_COLOR).next_to(self.commiter, DOWN+RIGHT, buff = -0.3)
        ...
        self.opening_verifier = VGroup(self.opening, self.proof, self.commitment)
        self.opening_verifier.generate_target()
        self.opening_verifier.target.next_to(self.verifier_label, DOWN)
        scene.play(MoveToTarget(self.opening_verifier))
        self.opening.generate_target()
        self.opening.target.shift(DOWN)
        self.proof.generate_target()
        self.proof.target.shift(DOWN*2)
