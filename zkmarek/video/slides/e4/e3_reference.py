from manim import LEFT, UP, DOWN, MathTex, Write, FadeOut, Text, RIGHT, ImageMobject, FadeIn, Rectangle, TransformMatchingShapes, Indicate, VGroup
...
class Episode3Reference(SlideBase):
    ...
    def animate_in(self, scene):
        ...
        self.definition = MathTex(r"e({{G_1}}, {{G_2}}) \rightarrow {{G_T}}", color = PRIMARY_COLOR).shift(UP*2)
        ...
        self.scale(0.65)
        scene.wait(1.5)
        scene.play(Indicate(self.definition[1], color = SECONDARY_COLOR))
        scene.wait(0.7)
        scene.play(Indicate(self.definition[3], color = SECONDARY_COLOR))
    
        self.new_subsection(scene, "bilinearity", "data/sound/e4/slide0-4b.mp3")
        self.bilinearity = MathTex(r"e({{a}}\cdot {{G_1}}, {{b}} \cdot {{G_2}} ) \stackrel{?}{=} e({{c}} \cdot {{G_1}}, {{d}} \cdot {{G_2}})", color = SECONDARY_COLOR).next_to(self.definition, DOWN)
        ...
        scene.wait(1)
        scene.play(TransformMatchingShapes(VGroup(self.bilinearity2[2].copy(), self.bilinearity2[2]) ,self.bilinearity3))
        scene.wait(1)
        scene.play(FadeOut(self.bilinearity2, self.bilinearity3, self.definition, rectangle, text))
