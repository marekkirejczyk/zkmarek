...
class EthBalances(SlideBase):
    ...
    def construct(self):
        ...
        self.arrow_0_1 = (
            ...
            .next_to(self.finalized_blocks[0], RIGHT, buff=0.05)
        )
        ...
        self.arrow_1_2 = (
            ...
            .set_color(PRIMARY_COLOR)
            .next_to(self.finalized_group[1], RIGHT, buff=0.05)
        )
        ...
        self.transaction_data = Text(
            "transaction data \n ... \n receipt data \n ...",
            font=PRIMARY_FONT,
            font_size=28,
        )

        self.transaction_data1 = Text(
            "transaction data \n ... \n receipt data \n ...",
            font=PRIMARY_FONT,
            font_size=28,
        )
        self.transaction_data2 = Text(
            "transaction data \n ... \n receipt data \n ...",
            font=PRIMARY_FONT,
            font_size=28,
        )

        self.account_data1 = Text(
            "account data: \n\n 0x1234...1234: 0.5 ETH", font=PRIMARY_FONT, font_size=28
        )
        self.account_data1_more = (
            Text(
                " 0x3791...2468: 0.8 ETH \n 0x2651...8137: 3.0 ETH \n 0x9132...3817: 0.2 ETH \n ...",
                font=PRIMARY_FONT,
                font_size=28,
            )
            .next_to(self.account_data1[22], DOWN, buff=0.2)
            .shift(0.1 * RIGHT)
        )
        self.account_data1 = VGroup(self.account_data1, self.account_data1_more)
        self.account_data2 = Text(
            "account data: \n\n 0x8173...7318: 1.0 ETH", font=PRIMARY_FONT, font_size=28
        )
        self.account_data2_more = (
            Text(
                " 0x0124...1265: 0.9 ETH \n 0x3791...2468: 2.0 ETH \n 0x9712...1386: 1.1 ETH \n ...",
                font=PRIMARY_FONT,
                font_size=28,
            )
            .next_to(self.account_data2[22], DOWN, buff=0.2)
            .shift(0.1 * RIGHT)
        )
        ...
        self.question_mark = Text(
            "?", font=PRIMARY_FONT, color=PRIMARY_COLOR, font_size=28
        )
    ...
    def animate_in(self, scene):
        ...
        self.ethereum.target.scale(2).move_to(
            self.finalized_blocks[1].target.get_bottom() + RIGHT * 3 + UP
        )
        ...
        self.data_size = Text(
            "15 TB", color=PRIMARY_COLOR, font=PRIMARY_FONT, font_size=24
        ).shift(UP)

        scene.play(Write(self.account_data), run_time=1)
        scene.play(
            Create(self.account_data_more), Write(self.transaction_data), run_time=1
        )
        ...
        scene.play(
            FadeIn(side_block1),
            FadeIn(side_block2),
            Write(self.arrow_0_1),
            Write(self.arrow_1_2),
        )

        self.account_data1.scale(1 / 2).move_to(
            self.finalized_blocks[0].get_top() + DOWN * 0.9 + LEFT * 0.8
        )
        self.account_data2.scale(1 / 2).move_to(
            self.finalized_blocks[2].get_top() + DOWN * 0.9 + LEFT * 0.8
        )
        self.transaction_data1.scale(1 / 2).next_to(
            self.account_data1[0], RIGHT, buff=0.1
        ).shift(DOWN * 0.12)
        self.transaction_data2.scale(1 / 2).next_to(
            self.account_data2[0], RIGHT, buff=0.1
        ).shift(DOWN * 0.12)

        self.ethereum3.generate_target()
        self.ethereum3.target.move_to(
            self.finalized_group[2].get_bottom() + RIGHT * 1.5 + UP * 0.5
        )
        self.ethereum2.generate_target()
        self.ethereum2.target.move_to(
            self.finalized_blocks[0].get_bottom() + RIGHT * 1.5 + UP * 0.5
        )

        scene.play(
            Write(self.account_data1),
            Write(self.account_data2),
            Write(self.transaction_data1),
            Write(self.transaction_data2),
            MoveToTarget(self.ethereum3),
            MoveToTarget(self.ethereum2),
        )
        ...
        scene.play(
            Write(self.question_mark.next_to(self.computer, RIGHT, buff=0.1)),
            run_time=0.8,
        )
        scene.wait(3.2)
        all_vecs = VGroup(
            self.transaction_data1, self.transaction_data, self.transaction_data2
        )
        for i in range(len(all_vecs)):
            scene.play(
                Indicate(all_vecs[i], color=SECONDARY_COLOR, scale_factor=1.2),
                run_time=0.3,
            )
        scene.wait(1)
        ...
        self.new_subsection(
            scene, "transaction included", "data/sound/e6/slide2-0c.mp3"
        )
        scene.wait(3)
        account_indications = [
            self.account_data2_more[0:20],
            self.account_data2_more[20:40],
            self.account_data2_more[40:60],
        ]
