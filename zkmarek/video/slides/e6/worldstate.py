...
class SimplifiedWorldState(VGroup):
    def __init__(self):
        ...
        value_header = Text(
            "Values", font=PRIMARY_FONT, font_size=24, color=PRIMARY_COLOR
        )
        title = Text(
            "Simplified State Trie",
            font=PRIMARY_FONT,
            font_size=25,
            color=PRIMARY_COLOR,
        )
        ...
        for row_idx, (key_row, value) in enumerate(zip(keys_data, values_data)):
            for col_idx, key in enumerate(key_row):
                key_cell = (
                    ...
                    .move_to(self.background.get_top())
                    .shift(
                        DOWN * (row_idx + 2) * (cell_height + spacing)
                        + LEFT * (num_cols - col_idx - 1.1) * (cell_width + 0.1)
                        + RIGHT * 0.7
                    )
                )

                key_text = Text(
                    key, font=PRIMARY_FONT, font_size=20, color=PRIMARY_COLOR
                ).move_to(key_cell)
            ...
            value_cell = (
                ...
                .move_to(self.background.get_top())
                .shift(DOWN * (row_idx + 2) * (cell_height + spacing) + RIGHT * 1.9)
            )

            value_text = Text(
                value, font=PRIMARY_FONT, font_size=20, color=PRIMARY_COLOR
            ).move_to(value_cell)
    ...
    def appear_table(self, scene):
        scene.play(
            Create(self.background),
            Create(self.key_cells),
            Create(self.value_cells),
            Create(self.key_header),
            Create(self.value_header),
            Create(self.title),
            run_time=1,
        )
        scene.wait(0.5)
