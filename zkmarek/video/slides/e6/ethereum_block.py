from manim import (Text, UP, VGroup, Group, FadeIn, MoveToTarget, LEFT, RIGHT, DOWN, Indicate, Write, Create,
                   RoundedRectangle, FadeOut, Arrow, DashedVMobject, StealthTip)
...
class EthereumBlock(SlideBase):
    ...
    def construct(self):
        ...
        self.labels = VGroup()
        rectangle = RoundedRectangle(width=1.5, height=0.6, color=PRIMARY_COLOR, corner_radius=0.15, fill_opacity=0.25, stroke_width = 0.0)
        
        self.rectangles = Group()
        for i, label in enumerate(labels):
            rectangle = rectangle.copy().move_to(self.block.get_center()).shift(LEFT*3.5+RIGHT*(i % 5)*1.8+UP*0.7)
            if i > 4:
        ...
        rectangle = RoundedRectangle(width=2.1, height=0.6, corner_radius=0.15, color=PRIMARY_COLOR, fill_opacity=0.25, stroke_width = 0.0)
        for i, trie in enumerate(tries):
            rectangle = rectangle.copy().move_to(self.block.get_center()).shift(LEFT*3.2+RIGHT*(i % 5)*2.22+UP*0.7).shift(DOWN*1.5)
    ...
    def animate_in(self, scene):
        ...
        self.block_header_whole.target.next_to(self.title_label, DOWN, buff = 0.1).shift(UP*0.6).scale(0.6)
        ...
        self.account_balance_node = RoundedRectangle(width = 3, height = 0.5, corner_radius=0.1, color = HIGHLIGHT_COLOR, fill_opacity = 0.15, stroke_width = 0.0)
        self.account_balance_node.move_to(self.nodes[5]).shift(RIGHT*1.5)
        
        address_rectangle = RoundedRectangle(width = 1.3, height = 0.4, corner_radius=0.1, color = HIGHLIGHT_COLOR, fill_opacity = 0.25, stroke_width = 0.0)
        address_rectangle.move_to(self.account_balance_node.get_center()).shift(LEFT*0.75)
        address_text = Text("address", font=PRIMARY_FONT, color=PRIMARY_COLOR, font_size=18).move_to(address_rectangle.get_center())
        self.address = VGroup(address_rectangle, address_text)
        
        balance_rectangle = RoundedRectangle(width = 1.3, height = 0.4, corner_radius=0.1, color = HIGHLIGHT_COLOR, fill_opacity = 0.25, stroke_width = 0.0)
        balance_rectangle.move_to(self.account_balance_node.get_center()).shift(RIGHT*0.75)
        balance_text = Text("balance", font=PRIMARY_FONT, color=PRIMARY_COLOR, font_size=18).move_to(balance_rectangle.get_center())
        ...
        rectangle_all = RoundedRectangle(width=1.05, height=0.4, corner_radius=0.1, color=HIGHLIGHT_COLOR, fill_opacity=0.25, stroke_width = 0.0)
        rectangle_storage = RoundedRectangle(width=1.55, height=0.4, corner_radius=0.1, color=HIGHLIGHT_COLOR, fill_opacity=0.25, stroke_width = 0.0)
        
        for i, label in enumerate(labels_state_trie):
            if i == 0:
                rectangle = rectangle_all.copy().move_to(self.rectangle_state_trie.get_center()).shift(LEFT*2.9)
            elif i < 3:
                rectangle = rectangle_all.copy().next_to(self.rectangles_state_trie[i-1], RIGHT, buff = 0.13)
            elif i>2:
                rectangle = rectangle_storage.copy().next_to(self.rectangles_state_trie[i-1], RIGHT, buff = 0.13)
            
            label_text = Text(label, font=PRIMARY_FONT, color=PRIMARY_COLOR, font_size=16).move_to(rectangle.get_center())
        ...
        scene.play(Indicate(self.tries[1], color = SECONDARY_COLOR, scale_factor=1.5),
                   Indicate(self.rectangles_tries[1], color = SECONDARY_COLOR), run_time=0.8)
        scene.wait(0.5)
        scene.play(Indicate(self.tries[2], color = SECONDARY_COLOR, scale_factor=1.5),
                   Indicate(self.rectangles_tries[2], color = SECONDARY_COLOR), run_time=0.8)
        scene.wait(0.5)
        scene.play(Indicate(self.tries[3], color = SECONDARY_COLOR, scale_factor=1.5),
                   Indicate(self.rectangles_tries[3], color = SECONDARY_COLOR), run_time=0.8)
        scene.wait(0.5)
    ...
    def create_state_trie(self):
        node1_0 = RoundedRectangle(corner_radius=0.15, height = 0.5, width = 0.5, color = HIGHLIGHT_COLOR, fill_opacity = 0.25, stroke_width = 0.0)
    ...
    def create_storage_trie(self):
        node1_0 = RoundedRectangle(corner_radius=0.12, height = 0.3, width = 0.3, color = HIGHLIGHT_COLOR, fill_opacity = 0.25, stroke_width = 0.0)
        ...
        arrow1 = create_arrow(self.labels_state_trie[4], node1_0, dash_density=15, add_down_shift=True)
        arrow2 = create_arrow(self.labels_state_trie[4], node1_1, dash_density=15, add_down_shift=True)
