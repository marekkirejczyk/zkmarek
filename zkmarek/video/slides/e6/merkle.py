...
class MerkleTree(SlideBase):
    ...
    def construct(self):
        ...
        self.arrow_0_1 = Arrow(self.finalized_blocks[0].get_right(), self.finalized_blocks[1].get_left(), color = PRIMARY_COLOR, tip_shape = StealthTip, stroke_width=2, 
                               max_tip_length_to_length_ratio=0.15).set_color_by_gradient([BLUE_B, GREEN_E])
        self.arrow_1_2 = Arrow(self.finalized_blocks[1].get_right(), self.finalized_blocks[2].get_left(), color = PRIMARY_COLOR, tip_shape = StealthTip, stroke_width=2,
                               max_tip_length_to_length_ratio=0.15).set_color_by_gradient([BLUE_B, GREEN_E])
        ...
        self.account_data1_more = Text("0x3791...2468: 0.8 ETH \n 0x2651...8137: 3 ETH \n 0x9132...3817: 0.2 ETH \n ...]", color = PRIMARY_COLOR, font=PRIMARY_FONT, font_size = 22).next_to(self.account_data1[18], DOWN, buff = 0.2)
        self.account_data1 = VGroup(self.account_data1, self.account_data1_more)
        self.account_data2 = Text("account data: [0x8167...7316", color = PRIMARY_COLOR, font=PRIMARY_FONT, font_size = 22)
        self.account_data2_more = Text("0x0124...1265: 0.9 ETH \n 0x3791...2468: 2 ETH \n 0x9712...1386: 1.1 ETH \n ...]", color = PRIMARY_COLOR, font=PRIMARY_FONT, font_size = 22).next_to(self.account_data2[18], DOWN, buff = 0.2)
    ...
    def animate_in(self, scene):
        ...
        self.arrow_0_1.target.scale(0.5).shift(LEFT*1.5)
        self.arrow_1_2.generate_target()
        self.arrow_1_2.target.scale(0.5).shift(RIGHT*1.5)
        ...
        self.account_data_more.next_to(self.account_data[22], DOWN, buff = 0.2)
        self.transaction_data.next_to(self.account_data, DOWN, buff = 1.5)  
        data_vecs = VGroup(self.transaction_data, self.account_data, self.account_data_more, self.dots)
        ...
        self.arrow_0_1.generate_target()
        self.arrow_0_1.target.scale(2).shift(RIGHT*1.5)
        self.arrow_1_2.generate_target()
        self.arrow_1_2.target.scale(2).shift(LEFT*1.5)
        ...
        scene.play(Write(self.account_data1), Write(self.account_data2), Write(self.transaction_data1), Write(self.transaction_data2), Create(self.dots2), Create(self.dots3))
        ...
        scene.wait(3)
        account_indications = [self.account_data1[15:25], self.account_data[15:25], self.account_data2[15:25]]
        ...
        scene.play(FadeOut(self.computer, main_block, side_block1, side_block2, self.finalized_group, self.eth_logos, self.transaction_data1, 
                           self.transaction_data2, self.account_data1, self.account_data2, self.dots2, self.dots3, self.arrow_0_1, self.arrow_1_2))
        ...
        for i in range(8):
            self.vector_8element[2*i+1].next_to(self.merkle24_nodes_level3[i], DOWN, buff = 0.35)
        ...
        self.MPT.leaf1.generate_target()
        self.MPT.leaf1.target.shift(LEFT*3+DOWN*1).scale(1/2.5).shift(LEFT*0.8).scale(0.7)
        everything_but_leaf.generate_target()
        ...
        self.proof_formula3.target.shift(RIGHT*1.5)
        scene.play(MoveToTarget(self.proof_formula3))
        self.thousand_proofs = MathTex(r"1000 \times", color = PRIMARY_COLOR, font_size = 30).next_to(self.proof_formula3, LEFT, buff = 0.1)
        self.thousand_size = MathTex(r"\sim 5 \ \mathrm{MB}", color = SECONDARY_COLOR, font_size = 30).next_to(self.proof_formula3, DOWN, buff = 0.1)
    ...
    def hashes_of_8_element(self, scene):
        ...
        scene.wait(1.5)
        for i in range(8):
            scene.play(TransformMatchingShapes(self.level3_hashes_not_numerical[i], self.level_3_hashes[i]), run_time=0.2)
    ...
    def size_16children(self, scene):
        ...
        scene.wait(3)
        self.proof_formula2 = MathTex(r"\text{Proof size} = \log_2({{n}}) \cdot \mathrm{HashSize}", color = PRIMARY_COLOR, font_size = 35)
        self.proof_formula2.to_edge(LEFT+UP).shift(DOWN)
        scene.play(FadeIn(self.proof_formula2))
        scene.wait(1.7)
        self.proof_formula3 = MathTex(r"\text{Proof size} = \log_2({{n}}) \cdot 32 \ \mathrm{B}", color = PRIMARY_COLOR, font_size = 35)
