from manim import (VGroup, Group, Text, ImageMobject, UP, LEFT, RIGHT, DOWN, Write, Create, MoveToTarget, Indicate, 
                   FadeOut, MathTex, RoundedRectangle, Transform, TransformMatchingShapes, Arrow, StealthTip,
                   FadeIn, DashedVMobject)
...
class MerkleTree(SlideBase):
    ...
    def animate_out(self, scene):
        scene.play(FadeOut(self.title_merkle_proof, self.merkle_tree_all))
    ...
    def update_nodes(self, scene):
        index_of_the_node = 0
        new_value = 9
        new_numerical_value = Text(str(new_value), color = PRIMARY_COLOR, font_size = 26, font=PRIMARY_FONT).move_to(self.account_group_8_elements[0].get_center())
    ...
    def merkle_proof(self, scene):     
        ...
        self.hashes_on_path = [self.level_2_hashes[1], self.new_hashes[2], self.new_hashes[3]]
        ...
        for i in range(len(nodes_needed_to_proof)):
            ...
            node.set_color(HIGHLIGHT2_COLOR).next_to(self.merkle_tree_2_4, DOWN, buff = 0.3).shift(LEFT*2+RIGHT*2*i)
            hash.move_to(node.get_center())
            scene.play(TransformMatchingShapes(VGroup(nodes_needed_to_proof[i], hashes_needed_to_proof[i]), VGroup(node, hash)), run_time=0.3)
        scene.wait(2.2)
    ...
    def calculate_merkle_proof(self, scene):
        ...
        scene.wait(1)
        scene.play(TransformMatchingShapes(VGroup(hash_sibling_leaf, self.level_2_hashes[1].copy()), parent_hash), self.level_2_hashes[1].animate.set_opacity(1.0).set_color(SECONDARY_COLOR), run_time=0.9)
        scene.wait(0.4)
        ...
        scene.wait(0.4)
        scene.play(TransformMatchingShapes(VGroup(parent_sibling_hash, self.new_hashes[2].copy()), grandparent_hash), self.new_hashes[2].animate.set_opacity(1.0).set_color(SECONDARY_COLOR), run_time=0.7)
        scene.wait(0.2)
        ...
        scene.wait(0.2)
        scene.play(TransformMatchingShapes(VGroup(grandparent_sibling_hash, self.new_hashes[3].copy()), root_hash), self.new_hashes[3].animate.set_opacity(1.0).set_color(SECONDARY_COLOR), run_time=0.7)
