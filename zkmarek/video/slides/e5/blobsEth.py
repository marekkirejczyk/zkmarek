...
class BlobsBlockchain(SlideBase):
    ...
    def construct(self):
        ...
        self.blob_field.set_color_by_gradient([GOLD_E, GREEN_E, BLUE_E]).next_to(self.block1_group, UP, buff = 0)
        ...
        self.commitment_hash.move_to(self.blob.get_bottom()).shift(UP+RIGHT*0.75)
        
        
        self.vector = MathTex(r"\boldsymbol{\left[ {{y_0}}, {{y_1}}, \cdots {{y_{4095}}}   \right]}", color = RED_D, font_size = 50).next_to(self.blob, UP)
    ...
    def animate_in(self, scene):
        ...
        self.new_subsection(scene, "transaction 0x03", "data/sound/e5/slide5-1.mp3")
        scene.play(FadeIn(self.commitment, self.vector))
        scene.wait(1)
        scene.play(Indicate(self.block1_group, color = GOLD_E), run_time=0.5)
        scene.play(Indicate(self.block2_group, color = GOLD_E), run_time=0.5)
        scene.play(Indicate(self.block3_group, color = GOLD_E), run_time=0.5)
        ...
        self.commitment.target.scale(0.6).shift(LEFT)
        scene.play(TransformMatchingShapes(self.commitment.copy(), self.commitment_hash), MoveToTarget(self.commitment), run_time=1)
        
        self.new_subsection(scene, "available for 4096 epochs", "data/sound/e5/slide5-3.mp3")
        scene.wait(1.5)
        scene.play(Indicate(self.vector), run_time=2)
        scene.wait(1.5)
    ...
    def animate_out(self, scene):
        scene.play(FadeOut(self.block1_group, self.blob_field, self.blob, self.block2_group, self.block3_group, self.commitment, self.commitment_hash, self.title_text, self.arrow, self.arrow2))
