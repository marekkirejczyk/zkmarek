...
class Previously(SlideBase):
    ...
    def construct(self):
        self.title_label = Text("Vector commitment in 90 s", font = PRIMARY_FONT, color = PRIMARY_COLOR, font_size = 60).to_edge(UP).shift(UP*5)
        self.polynomial_chart = DiscreetePolynomialChart(p = 41, f = poly, label = "r", include_numbers=False, dot_color=SECONDARY_COLOR).scale(0.8).shift(3*LEFT+DOWN*3.2)
        ...
        self.prover = ImageMobject("data/images/person.png").to_edge(LEFT).shift(RIGHT+UP*4)
        self.verifier = ImageMobject("data/images/person_blue.png").to_edge(RIGHT).shift(LEFT+UP*4)
        self.commiter_label = Text("Prover", font = PRIMARY_FONT, color = PRIMARY_COLOR, font_size = 32).next_to(self.prover, DOWN)
        self.verifier_label = Text("Verifier", font = PRIMARY_FONT, color = PRIMARY_COLOR, font_size = 32).next_to(self.verifier, DOWN)

        self.polynomial_opening_label = MathTex(r"{{r(x)}} = {{p(x)}} - {{y_0}}", color = PRIMARY_COLOR, font_size=45).next_to(self.polynomial_chart, UP, buff = 0.5)
        
        self.opening = MathTex(r"{{}}p({{x_0}}) {{}} = {{y_0}}", font_size=55, color=PRIMARY_COLOR).next_to(self.verifier_label, DOWN, buff = 0.7).shift(DOWN*0.7)

        self.opening2 = MathTex(r"{{r(x)}} = {{p(x)}} - {{y_0}} {{}}", font_size=55, color=PRIMARY_COLOR).next_to(self.opening, DOWN, buff = 1.2)   
        self.new_polynomial_label = Text("new polynomial", color = GREEN_E, font_size=23, font = PRIMARY_FONT).next_to(self.opening2, LEFT, buff = 1.7).shift(UP*0.14)
        self.opening3 = MathTex(r"{{a}}{{(x-x_1)}}{{(x - x_2)}}{{(x-x_0)}} = {{p(x)}} - {{y_0}} {{}}", font_size=50, color=PRIMARY_COLOR)
    ...
    def animate_in(self, scene):
        ...
        self.pairing_verifiaction_0.next_to(self.quotient_deriviation_3, DOWN, buff = 0.5).shift(DOWN*3)
        self.pairing_verifiaction_1.next_to(self.quotient_deriviation_3, DOWN, buff = 0.5).shift(DOWN*3)
        self.pairing_verifiaction_2.next_to(self.quotient_deriviation_3, DOWN, buff = 0.5).shift(DOWN*3)
        self.pairing_verifiaction_2a.next_to(self.quotient_deriviation_3, DOWN, buff = 0.5).shift(DOWN*3)
    ...
    def y0_x0(self, scene):
        ...
        scene.wait(1)
        scene.play(FadeIn(self.polynomial_chart), Write(self.polynomial_label), run_time=1)
        ...
        scene.play(MoveToTarget(self.commitment_sent), MoveToTarget(self.proof), MoveToTarget(self.opening))
