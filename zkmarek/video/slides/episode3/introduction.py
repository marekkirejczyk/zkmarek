...
class Introduction(SlideBase):
    ...
    def construct(self):
        ...
        self.polynomial3 =  MathTex(r"f(\tau) = \tau^{24}-84\tau^{10}-16", font_size = 60, color = SECONDARY_COLOR).shift(RIGHT*2+DOWN*2)
        self.polynomial4 =  MathTex(r"g(\tau) = \tau^{32}-18\tau^{20}+63\tau^3", font_size = 60, color = SECONDARY_COLOR).shift(LEFT*2+DOWN*2)
        self.polynomial5 =  MathTex(r"h(\tau) = \tau^{18}-17\tau^{19}+6\tau", font_size = 60, color = SECONDARY_COLOR).shift(DOWN*3)
