...
class ThreeLevelVerkleTree(SlideBase):
    ...
    def construct(self):
        ...
        self.internal_node2 = VGroup(*self.internal_node2).arrange(RIGHT, buff = 0.15).shift(DOWN*1.5+RIGHT*1)
        ...
        self.commitment_to_C0 = rectangle.copy().scale(3).set_color(HIGHLIGHT_COLOR).next_to(self.internal_node, UP, buff = 0.5).shift(UP*0.5)
        self.commitment_to_C255 = rectangle.copy().scale(1.5).set_color(HIGHLIGHT_COLOR).next_to(self.internal_node2, UP, buff = 0.6)
        ...
        self.other_hashes_rec = VGroup(*self.other_hashes_rec).arrange(RIGHT, buff = 0.3)
        ...
        self.rectangle_with_hashes = RoundedRectangle(corner_radius=0.1, width = 3.5, height = 2, fill_opacity = 0.15, stroke_width = 0.0).set_color(HIGHLIGHT_COLOR)
        self.rectangle_node_commitments2 = self.rectangle_with_hashes.copy().set_color(HIGHLIGHT_COLOR).move_to(self.commitments_copy_recs.get_center()).shift(UP * 0.5)
    ...
    def animate_in(self, scene):
        ...
        self.rectangle_with_hashes.move_to(self.hashaes_of_commitments.get_center()).shift(DOWN*0.3)
        ...
        self.whole_tree_until_C10.generate_target()
        self.whole_tree_until_C10.target.shift(LEFT*3)
        scene.play(MoveToTarget(self.whole_tree_until_C10), run_time=1)
        self.arrow_commitment_C01 = []
        for rectangle in self.hashaes_of_commitments_rec:
            self.create_arrow(self.commitment_C01_rec, rectangle, self.arrow_commitment_C01)
        self.other_commitments.next_to(self.commitment_C01, RIGHT, buff = 0.4)
        self.right_commitments.next_to(self.other_commitments[1], DOWN, buff = 0.2).shift(RIGHT * 1.8)
        scene.play(Create(self.right_commitments), run_time=1)
        ...
        self.whole_tree_until_C10.add(self.final_parent_node,*arrows_final, self.right_commitments)
