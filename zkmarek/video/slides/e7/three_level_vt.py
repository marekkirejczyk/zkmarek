...
class ThreeLevelVerkleTree(SlideBase):
    ...
    def construct(self):
        ...
        rectangle = RoundedRectangle(corner_radius=0.1, width = 0.5, height = 0.3, fill_opacity = 0.25,  stroke_width = 0.0).set_color(SECONDARY_COLOR)
        
        self.internal_node = [rectangle.copy().scale(2.) for _ in range(2)]
        self.internal_node = VGroup(*self.internal_node).arrange(RIGHT, buff = 0.6).shift(DOWN * 1.5)
        self.internal_node2 = [rectangle.copy().scale(1.) for _ in range(2)]
        self.internal_node2 = VGroup(*self.internal_node2).arrange(RIGHT, buff = 0.3).shift(DOWN*1.7+RIGHT*1.)
        
        txs = [r"a_0", r"a_{255}"]
        txs2 = [r"", r""]
        self.node1_texts = [MathTex(txs[i], font_size=40, color=PRIMARY_COLOR).move_to(self.internal_node[i]) for i in range(len(txs))]
        ...
        self.internal_node2_whole = VGroup(self.internal_node2, self.node2_texts,
                                          self.commitment_255,
                                          *self.arrows_C255).shift(RIGHT * 0.1)
        ...
        self.rectangle_node_commitments = RoundedRectangle(corner_radius=0.1, width = 3.5   , height = 0.8, fill_opacity = 0.15, stroke_width = 0.0).set_color(HIGHLIGHT_COLOR)
        ...
        self.hashaes_of_commitments_rec = [rectangle.copy().scale(1.8).set_color(PRIMARY_COLOR) for _ in range(2)]
        self.hashaes_of_commitments_rec = VGroup(*self.hashaes_of_commitments_rec).arrange(RIGHT, buff = 1.)
        iss = ["0", "255"]
        self.text_hashes_of_commitments = [MathTex(f"H(C^0_{{{iss[i]}}})", font_size=22, color=PRIMARY_COLOR).move_to(self.hashaes_of_commitments_rec[i]) for i in range(len(self.hashaes_of_commitments_rec))]
        ...
        self.commiemtnt_c01_text = MathTex(r"C_0^1", font_size=26, color=PRIMARY_COLOR).move_to(self.commitment_C01_rec.get_center())

        self.commitment_C01 = VGroup(self.commitment_C01_rec, self.commiemtnt_c01_text)
        
        self.other_commitments = [rectangle.copy().scale(1.8).set_color(HIGHLIGHT_COLOR) for _ in range(1)]
        self.other_commitments = VGroup(*self.other_commitments).arrange(RIGHT, buff = 3.5)
        self.text_other_commitments = [MathTex(f"C_{{{iss[i+1]}}}^1", font_size=26, color=PRIMARY_COLOR).move_to(self.other_commitments[i]) for i in range(len(self.other_commitments))]

        self.other_commitments.add(*self.text_other_commitments)
        self.text_other_commitments = VGroup(*self.text_other_commitments)
        
        self.other_hashes_rec = [rectangle.copy().stretch(1, 2).scale(1.8).set_color(HIGHLIGHT_COLOR) for _ in range(2)]
        self.other_hashes_rec = VGroup(*self.other_hashes_rec).arrange(RIGHT, buff = 2.5)
        self.text_other_hashes = [MathTex(f"C_{{{iss[i]}}}^1", font_size=26, color=PRIMARY_COLOR).move_to(self.other_hashes_rec[i]) for i in range(len(self.other_hashes_rec))]
        ...
        self.single_commitment = Text("a single commitment has...", font=PRIMARY_FONT, font_size=20, color=PRIMARY_COLOR)
        ...
        self.values2_to_24 = MathTex(r"({{256}})^3 = ({{2^8}})^3 = {{2^{24}}}", font_size=45, color = PRIMARY_COLOR)
        self.values_four_level = MathTex(r"({{256}})^4 = ({{2^8}})^4 = {{2^{32}}} = {{...}}", color = PRIMARY_COLOR)
        
        hashes_copy_recs = [rectangle.copy().scale(1.8).set_color(HIGHLIGHT_COLOR) for _ in range(2)]
        self.hashes_copy_recs = VGroup(*hashes_copy_recs).arrange(RIGHT, buff = 1.)
    
        self.rectangle_with_hashes = RoundedRectangle(corner_radius=0.1, width = 3.5, height = 0.8, fill_opacity = 0.15, stroke_width = 0.0).set_color(HIGHLIGHT_COLOR)
        self.rectangle_node_commitments2 = self.rectangle_with_hashes.copy().set_color(HIGHLIGHT_COLOR).move_to(self.hashes_copy_recs.get_center())
        
        values = self.internal_node2.copy().next_to(self.hashes_copy_recs[0], DOWN, buff = 0.65)
        values2 = self.internal_node2.copy().next_to(self.hashes_copy_recs[1], DOWN, buff = 0.65)
        arrows = []
        ...
        self.right_commitments = VGroup(*values, *values2, *arrows, *self.hashes_copy_recs,
                                        ...
                                        self.cdots_right_nodes2, self.cdots_right_nodes3)
        self.right_commitments_opacity = VGroup(*values, *values2, *self.hashes_copy_recs)
    ...
    def animate_in(self, scene):
        ...
        scene.play(Create(self.internal_node), Write(self.node1_texts), Write(self.cdots_node1), run_time=1)
        ...
        scene.wait(0.5)
        scene.play(Indicate(self.internal_node[0], color = PRIMARY_COLOR),
                   Indicate(self.node1_texts[0], color = PRIMARY_COLOR), run_time=0.5)
        scene.play(Indicate(self.cdots_node1, color = PRIMARY_COLOR), run_time=0.5)
        scene.play(Indicate(self.internal_node[1], color = PRIMARY_COLOR),
                   Indicate(self.node1_texts[1], color = PRIMARY_COLOR), run_time=0.5)
        scene.wait(1.5)
        scene.play(Indicate(self.commitment_to_c0_text, color = PRIMARY_COLOR), run_time=1)
        ...
        self.internal_node_whole.target.scale(0.5).next_to(self.internal_node2_whole, LEFT, buff = 0.6)
        scene.play(MoveToTarget(self.internal_node_whole), run_time=1)
        scene.play(FadeIn(self.internal_node2_whole, self.cdots_node2, self.cdots_node3), run_time=1)
        ...
        scene.play(Indicate(self.rectangle_node_commitments, color = PRIMARY_COLOR), run_time=1)
        scene.wait(1)
        ...
        all_values = Text("65536 values", font=PRIMARY_FONT, font_size=30, color=PRIMARY_COLOR)
        ...
        scene.wait(1.5)
        scene.play(Indicate(self.commitmentcdots),
        ...
        scene.wait(0.5)
        self.commitment_to_ec_values.next_to(self.commitmentcdots, UP, buff = 1.0).shift(LEFT * 0.5+UP)
        ...
        scene.play(self.commitment_size_ipa.animate.shift(UP*0.8), self.commitment_size_brace.animate.shift(UP*0.8), run_time=1)
        hash = Text("hash()", color = PRIMARY_COLOR, font = PRIMARY_FONT, font_size = 20).next_to(self.commitment_size_ipa, DOWN, buff = 0.7)
        
        scene.play(FadeIn(hash),
                   run_time=1)
        ...
        self.whole_tree_until_C10 = VGroup(self.internal_node, self.node1_texts, 
                                          self.arrows_C0, self.internal_node2, self.node2_texts, self.cdots_node3, self.cdots_node2, self.cdots_node1,
                                          *self.arrows_C255, self.rectangle_node_commitments,
                                          *self.arrows_C0, *self.arrows_C255, self.commitmentcdots, self.commitment_255, self.commitment_to_C0, self.commitment_to_c0_text)
        ...
        self.commitment_C01.target.next_to(self.commitment_to_C0, UP, buff = 0.9).shift(LEFT * 1.25)
        self.whole_tree_until_C10.generate_target()
        ...
        self.right_commitments.next_to(self.other_commitments[0], DOWN, buff = 0.73)
        self.rectangle_on_parent_commitments.move_to(self.other_commitments.get_center()).shift(LEFT * 2.2)
        ...
        for node in self.hashes_copy_recs:
            self.create_arrow(self.other_commitments[0], node, self.arrow_commitment_C01)
        ...
        scene.play(Create(self.other_hashes), FadeOut(self.other_commitments, self.commitment_C01), run_time=1)
        scene.wait(1)
        ...
        self.whole_tree_until_C10.add(self.final_parent_node,*arrows_final, self.arrow_commitment_C01,
                                      self.rectangle_on_parent_commitments, self.right_commitments, self.cdots_hashes2)
        ...
        nodes = VGroup(self.internal_node2, self.internal_node, self.commitments, 
                 self.right_commitments_opacity, self.other_hashes_rec, self.final_parent_node_rec)
        
        texts = VGroup(self.node2_texts, self.node1_texts, self.commitment_to_c0_text,
                       self.commitment_to_c255_text, self.final_parent_node_text, *self.text_other_hashes)
        scene.play(nodes.animate.set_opacity(0.15),
                   texts.animate.set_opacity(0.15), run_time=2)
        
        self.new_subsection(scene, "simple level VT holds 256 vals", "data/sound/e7/slide4-11a.mp3")

        self.values256.next_to(self.whole_tree_until_C10, RIGHT, buff = 1.0).shift(RIGHT*0.5+UP*0.5)
        self.single_commitment.next_to(self.values256, UP, buff = 0.3)
        self.values2_to_8.next_to(self.whole_tree_until_C10, RIGHT, buff = 1.0).shift(RIGHT*0.5+UP*0.5)
        ...
        self.values_four_level.next_to(self.values2_to_8, DOWN, buff = 1.0).shift(RIGHT * 0.5).scale(0.7)
        scene.play(Write(self.values256), Write(self.single_commitment), run_time=1)
        scene.play(self.commitment_to_c0_text.animate.set_opacity(1.),
                   self.commitment_to_c255_text.animate.set_opacity(1.0), run_time=0.5)
        
        scene.play(Indicate(self.commitment_to_c0_text, color = PRIMARY_COLOR),
                   Indicate(self.commitment_to_c255_text, color = PRIMARY_COLOR), run_time=1.)
        ...
        scene.play(self.internal_node2.animate.set_opacity(0.4),
