...
class ThreeLevelVerkleTree(SlideBase):
    ...
    def construct(self):
        ...
        self.other_hashes_rec = VGroup(*self.other_hashes_rec).arrange(RIGHT, buff = 0.4)
    ...
    def animate_in(self, scene):
        ...
        self.whole_tree_until_C10.generate_target()
        self.whole_tree_until_C10.target.shift(LEFT*2.5)
        scene.play(MoveToTarget(self.whole_tree_until_C10), run_time=1)
        self.arrow_commitment_C01 = []
        for rectangle in self.hashaes_of_commitments_rec:
            self.create_arrow(self.commitment_C01_rec, rectangle, self.arrow_commitment_C01)
        self.other_commitments.next_to(self.commitment_C01, RIGHT, buff = 0.4)
        self.right_commitments.next_to(self.other_commitments[1], DOWN, buff = 0.2).shift(RIGHT * 1.35)
        ...
        self.four_level.next_to(self.values2_to_8, DOWN, buff = 1.0)
    ...
    def animate_out(self, scene):
        scene.play(FadeOut(self.title_label, self.values2_to_8, 
                           self.four_level, self.block, self.ethereum_addresses, self.number, self.whole_tree_until_C10, self.values_four_level), run_time=1)
    ...
    def generate_multi_level_snapshot(self, scene):
        ...
        self.cdots = Text(r"...", font=PRIMARY_FONT, font_size=40, color=PRIMARY_COLOR).move_to(self.first_block[5])
        ...
        self.cdots2 = self.cdots.copy().move_to(self.second_block[5])
        self.cdots3 = self.cdots.copy().move_to(self.third_block[5])
        ...
        arrows.remove(*arrows[3:4], *arrows[13:14], *arrows[23:24])
    ...
    def create_ethereum_addresses_counter(self, scene):
        scene.play(FadeOut(self.values256, self.single_commitment))
        ...
        self.block.next_to(self.four_level, DOWN, buff = 1.0)
