...
class PreviouslyVectorCommitment(SlideBase):
    ...
    def construct(self):
        ...
        self.rectangles_values = VGroup(*rectangles_of_values).next_to(self.prover, DOWN, buff = 1.5).arrange(RIGHT, buff=0.07).shift(LEFT*2+DOWN*0.3)
        for i in range(8):
    ...
    def animate_in(self, scene):
        ...
        scene.wait(1)
        values_y = [4, 16, 22, 23, 22, 20, 20, 20]
        self.points_dots = []
        values = [
        ...
        scene.wait(1)
        scene.play(Write(self.data_points), run_time=1)
    ...
    def animate_out(self, scene):
        scene.play(FadeOut(self.title_label, self.data_vector, self.envelope, self.arrow_data_interpolation, self.arrow_interpolation_vector, 
                           self.opening, self.proof_pi, self.verifier, self.prover, self.polynomial, self.verifier_label, 
