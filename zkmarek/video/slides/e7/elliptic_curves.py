from manim import (FadeIn, CurvedArrow, Write, Text, Create, RIGHT, UP, DOWN, LEFT, MathTex, ImageMobject, StealthTip, FadeOut,
                   Circle, MoveToTarget, Indicate, ApplyWave)
...
class EllipticCurves(SlideBase):
    ...
    def construct(self):
        ...
        self.chart_scalar = DiscreetePolynomialChart(p=41, f=poly, label = "r", include_numbers=False, dot_color=SECONDARY_COLOR).scale(0.7).shift(RIGHT*3+DOWN*0.5)
        ...
        self.chart_bander = PolynomialOnCurve(curve=BanderSnatch, dot_color=HIGHLIGHT_COLOR, label = "p", include_numbers=False).scale(0.7).shift(LEFT*3+DOWN*0.5)

        self.p = MathTex(r"{{p}} \approx 2^{381}", color = HIGHLIGHT_COLOR, font_size = 32).next_to(self.chart_ec, LEFT, buff = 2).shift(UP * 0.2)
        ...
        self.curve_ec = Text("BLS12-381", color = HIGHLIGHT_COLOR, font =  PRIMARY_FONT, font_size=34).next_to(self.chart_ec, RIGHT, buff = 0.3)
        self.curve_ec_bander = Text("BanderSnatch", color = HIGHLIGHT_COLOR, font =  PRIMARY_FONT, font_size=34).to_edge(DOWN).shift(LEFT * 3)
        ...
        self.formula_bls = MathTex(r"{{y^2}} = {{x^3}} + {{4}}", color = HIGHLIGHT_COLOR, font_size=34).next_to(self.curve_ec, DOWN).shift(DOWN*0.3)
        ...
        self.circle_gen = Circle(radius = 0.3, color = SECONDARY_COLOR).move_to(self.point_to_generator.get_center())
        self.point_to_generator_label = MathTex(r"{{G \cdot 1}}", color = SECONDARY_COLOR, font_size = 32).next_to(self.point_to_generator, RIGHT, buff = 0.5)
    ...
    def animate_in(self, scene):
        ...
        self.p.target.next_to(self.chart_ec.target, LEFT, buff = 1).shift(UP * 0.2)
        self.bytes_p2.generate_target()
        self.bytes_p2.target.shift(LEFT*3)
        scene.play(FadeOut(self.point_to_generator_label, self.circle_gen, self.point_to_generator), run_time=0.3)
        scene.play(MoveToTarget(self.chart_ec), MoveToTarget(self.curve_ec), MoveToTarget(self.p), 
                   MoveToTarget(self.bytes_p2), run_time=1)
