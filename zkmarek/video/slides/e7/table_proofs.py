...
class MerkleVerkleTable(VGroup):
    def __init__(self):
        ...
        headers = [
            ...
            Text("Verkle (IPA)", font=PRIMARY_FONT, font_size=22, color=PRIMARY_COLOR),
            Text("Verkle (KZG)", font=PRIMARY_FONT, font_size=22, color=PRIMARY_COLOR)
        ]
        ...
        for i, (cat, m_val, ipa_val, kzg_val) in enumerate(zip(categories, merkle_values, ipa_values, kzg_values)):
            base_y = self.background.get_top()+self.background.get_left() + DOWN * (i + 1.5) * (cell_height + spacing)+RIGHT*1.5
            ...
            def make_cell(text, width=cell_width, color=PRIMARY_COLOR):
                ...
                return cell, Text(text, font=PRIMARY_FONT, font_size=18, color=color).move_to(cell)
        ...
        self.add(self.background, self.headers, self.merkle_col, self.ipa_col, self.kzg_col, self.vec)
