...
class Wallet(VGroup):
    ...
    def animate_private_key(self, scene, key, run_time: float = 1, font_size=24):
        new_secret_key_value: Text = Text(f"{key}", font_size=font_size, font=PRIMARY_FONT, color=SECONDARY_COLOR)
    ...
    def animate_address_value(self, scene: Scene, address_value: str, *with_animations: Animation):
        ...
        scene.play(ReplacementTransform(self.address_value, new_address_value), *with_animations)
